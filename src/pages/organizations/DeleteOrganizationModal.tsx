...
import { OrganizationInput, useDeleteOrganization } from "../../api/organization";

type Props = {
  organization: OrganizationInput | null;
};

const DeleteOrganizationModal = ({ organization }: Props) => {
  const { mutate, isLoading, error } = useDeleteOrganization(() => {
    document.querySelector<HTMLDialogElement>(".delete-organization-modal")?.close();
  ...
  return (
    <dialog className="delete-organization-modal modal">
      <div className="modal-box bg-white rounded-2xl shadow-lg w-full max-w-md border border-orange-200">
        ...
        <p className="text-gray-800 mb-4">
          ...
          <strong className="text-green-600">{organization?.name}</strong>?
        </p>
